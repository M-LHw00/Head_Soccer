...
For understanding the basics of pygame: main loop, blit, etc.
'''
import pygame
import game
import random
...
#https://www.1001fonts.com/husky-stash-font.html for the fonts
def gameIntro():
    running = True
...
def chooseScreen():
    ...
    while running:
        ...
        if player1=='Random' and player2=='Random':
            ...
            pygame.draw.rect(gamePlay.screen,(198,40,40),(545,229,(charDict[player2][2]/10)*45+5,9))
            pygame.draw.rect(gamePlay.screen,(198,40,40),(545,263,(charDict[player2][3]/0.8)*45+5,9))
            pygame.draw.rect(gamePlay.screen,(198,40,40),(545,296,(charDict[player2][4]/-20)*45+5,9))
        elif player2 == 'Random':
            gamePlay.screen.blit(qMark,(502,115))
            player1List = charDict[player1][0]
            image1 = player1List[spriteCounter1]
            gamePlay.screen.blit(image1,(147,115))
            pygame.draw.rect(gamePlay.screen,(198,40,40),(190,230,(charDict[player1][2]/10)*45+5,9))
            pygame.draw.rect(gamePlay.screen,(198,40,40),(190,264,(charDict[player1][3]/0.8)*45+5,9))
            pygame.draw.rect(gamePlay.screen,(198,40,40),(190,297,(charDict[player1][4]/-20)*45+5,9))
        else:
            player1List = charDict[player1][0]
            player2List = charDict[player2][1]
            image1 = player1List[spriteCounter1]
...
def tournamentScreen(player,AI):
    ...
    while running:
        ...
        if 306<mouseX<406 and 194<mouseY<294:
            button = True
            if button:
                gamePlay.screen.blit(clickedlogoImage,(WIDTH//2-93,HEIGHT//2-14))
        else:
...
def loseScreen():
    ...
    while running:
        lostSound.play()
        gamePlay.screen.blit(backgroundSplash,(0,0))
        gamePlay.screen.blit(lost,(0,0))
        gamePlay.screen.blit(restart,(0,0))
        gamePlay.screen.blit(quitImg,(0,0))
        mouse = pygame.mouse.get_pos()
        mouseX = mouse[0]
        mouseY = mouse[1]
        if 288<mouseX<412 and 206<mouseY<249:
            button = True
            if button:
                gamePlay.screen.blit(restartClicked,(0,1))
        else:
            button = False
        if 287<mouseX<411 and 267<mouseY<310:
            button1 = True
            if button1:
                gamePlay.screen.blit(quitClickedImg,(0,1))
        else:
            button1 = False
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                quit()
            elif 288<mouse[0]<412 and 206<mouse[1]<249 and event.type == pygame.MOUSEBUTTONDOWN:
                lostSound.stop()
                goalSound.play()
                goalSound.fadeout(1000)
                pygame.mixer.music.load('sound/champions.wav')
                ...
                newGame = restartGame()
                return newGame
            elif 287<mouse[0]<412 and 267<mouse[1]<310 and event.type == pygame.MOUSEBUTTONDOWN:
                running = False
                pygame.quit()
                quit()
...
def mainGame(gamePlay = gamePlay):
